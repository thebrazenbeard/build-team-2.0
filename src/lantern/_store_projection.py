...
class ProjectionMixin:
    ...
    def _project_link(self, record: RecordEnvelope) -> None:
        ...
        source = payload.get("source_record_id")
        target = payload.get("target_record_id")
        if not isinstance(link_type, str) or link_type not in _LINK_MATRIX:
            raise ValidationError("Invalid v1 link type")
        if not isinstance(source, str) or not isinstance(target, str):
            raise ValidationError("Link endpoints must be record IDs")
    ...
    def _project_state_event(self, record: RecordEnvelope) -> None:
        ...
        if not all(isinstance(value, str) for value in (subject, event_type, event_key)):
            raise ValidationError("StateEvent payload is incomplete")
        self._record_type(subject)
        ...
        self._connection.execute(
            ...
            values (?, ?, ?, ?, ?, ?)
            """,
            (record.record_id, subject, event_type, record.predecessor_record_id, event_key, canonical_json(details)),
        )
        self._connection.execute(
            """
    ...
    def _emit_state_event(
        ...
        *,
        ...
        actor_id: str = "lantern-system",
    ) -> str:
        existing = self._connection.execute("select record_id from state_events where event_key=?", (event_key,)).fetchone()
    ...
    def _emit_supersession_effects(self, successor: RecordEnvelope) -> None:
        ...
        dependents = self._connection.execute(
            ...
            select l.record_id as link_id, l.source_record_id as dependent_id,
                   r.record_type, r.project_id, r.lineage_key
            from links l
            join records r on r.record_id=l.source_record_id
            where l.link_type='DEPENDS_ON' and l.target_record_id=?
              and r.record_type in ('Assessment','Decision')
            order by l.source_record_id, l.record_id
            """,
            (predecessor,),
        ).fetchall()
        for row in dependents:
            if row["lineage_key"] is not None:
                ...
                if current is None or current["record_id"] != row["dependent_id"]:
                    continue
            self._emit_state_event(
                subject_record_id=str(row["dependent_id"]),
                ...
                details={
                    ...
                    "dependency_link_id": row["link_id"],
                    "trigger": "DIRECT_DEPENDS_ON_SUPERSESSION",
                },
            )
