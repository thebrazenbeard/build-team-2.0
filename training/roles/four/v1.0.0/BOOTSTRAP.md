...
9. Freeze the trained chat as a base with a qualification receipt that binds the exact training package version, repository/source commit, manifest digest, module digests, module results, and final result.
10. When a working chat is branched from the base, perform a fresh operational reorientation. Recover current governance, role map, assignments, leases, provider currentness, blockers, and checkpoints from their current authoritative sources. Never reuse the frozen base's training exercises as operational truth.
